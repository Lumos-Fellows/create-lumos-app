...
import type { GitRunner } from "../src/types.ts";
/**
...
describe("initializeGitRepository", () => {
  it("runs git init, add, and commit in order for standalone projects", async () => {
    const calls: Parameters<GitRunner>[] = [];
    const runner = mock.fn<GitRunner>(async (cmd, args, opts) => {
    ...
    const logger = { warn: mock.fn<(message: string) => void>() };
  ...
  it("skips projects already inside a Git repository", async () => {
    const runner = mock.fn<GitRunner>(async () => "/tmp/existing-repo");
    const logger = { warn: mock.fn<(message: string) => void>() };
  ...
  it("warns without throwing when git init fails", async () => {
    const runner = mock.fn<GitRunner>(async (_cmd, args) => {
      if (args[0] === "rev-parse") {
        throw new Error("not a git repository");
      }
      if (args[0] === "init") {
