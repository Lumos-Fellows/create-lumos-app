...
describe("Anti-slop integration", () => {
  it("enables every vendored generic rule as an error", () => {
    const config = z
      .object({ rules: z.record(z.string(), z.string()) })
      .parse(
        JSON.parse(
    ...
    it(`${framework} templates pass lint and share valid agent guidance for every integration combination`, () => {
      ...
      for (const template of templates) {
        for (let mask = 0; mask < 2 ** integrations.length; mask++) {
          ...
          for (const [index, name] of integrations.entries()) {
            options[name] = Boolean(mask & (1 << index));
          }
