...
import type { IntegrationOptions, OverlayOptions } from "./types.ts";
import { templatesDir } from "./utils.ts";

/**
 * Copy template directories into the project, then strip conditional sections.
 */
export function applyOverlay(projectPath: string, options: OverlayOptions) {
 ...
 */
function buildTagPatterns(tag: string) {
  return {
    removeSection: [
      new RegExp(
 ...
 */
function stripConditionals(dir: string, integrations: IntegrationOptions) {
