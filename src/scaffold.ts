...
import type { Framework, PackageManager, ScaffoldOptions } from "./types.ts";
import { isCurrentDir, run } from "./utils.ts";
...
export async function scaffold(options: ScaffoldOptions) {
  ...
  const needsTempDir =
    ...
    basename(process.cwd()) !== options.resolvedName;
  const scaffoldTarget =
    needsTempDir && options.resolvedName ? options.resolvedName : name;
