...
- E2E tests must run the generated lint script after dependencies and UI components are installed; template-only checks cannot validate downloaded components.
...
- Do not rely on agent hooks as the only enforcement. CI must run lint, unit tests, E2E tests, and the Changeset check.
