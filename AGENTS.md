...
- After changing `src/` or `templates/`, run `pnpm typecheck`, `pnpm test:unit`, `pnpm lint`, and `pnpm test`. `pnpm verify` runs the complete verification sequence and is also used by Codex and Claude Stop hooks. CI must enforce checks independently of the harness.
